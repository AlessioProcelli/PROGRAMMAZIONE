...
void Controller::drawShape(int x, int y, Brush &brush, std::shared_ptr<Shape> shape)
{
    ...
    //viene rimpicciolita per rendere la visualizzazione uniforme
...
{
    model->addState(*(model->getCurrentState()));
    shape->setStartCoordinates(x / 2, y / 2);//imposta le cordinate secondo l'immagine rimpicciolita
}
...
{
    matrix = matrix.scaled((matrix.width() / (scale - 1)) * scale,
                           (matrix.height() / (scale - 1)) * scale,
                           Qt::KeepAspectRatio);
}
...
void Controller::rotate(Direction direction)
{
    ...
    switch (direction) {
    case Direction::Left:
        rotateImg = turnLeft(img);
        break;

    case Direction::Right:
        rotateImg = turnRight(img);
        break;
